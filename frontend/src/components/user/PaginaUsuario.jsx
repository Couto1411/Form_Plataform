            ...
            return(
                <div key={element.id} className={forms.length>numero?'border-bottom':''}>
                    <MDBListGroupItem noBorders className='d-flex rounded-2 align-items-center'>
                        {numero}. <Link className='zoom' to="/forms" onClick={e=>{sessionStorage.setItem("formId",element.id)}}>{element.titulo}{element.dataEnviado?"("+element.dataEnviado+")":<></>}</Link>{element.derivados?.length?<i id={'icone'+element.id} aberto='F' onClick={e=>{toggleDerivados(element.id)}} className=" mx-2 fas fa-regular fa-angle-down"></i>:null}
                                ...
                                {element.derivados.map(item =>{
                                    return (
                                        <MDBListGroupItem key={'formderivado'+item.id} className='pb-0 formsDuplicates d-flex'>
                                            {item.id[0]?<>{item.titulo}</>:<Link className='formsDuplicates zoom' to={"/forms/"+item.id} onClick={e=>{sessionStorage.setItem("formId",element.id);sessionStorage.setItem("formDeId",item.id)}}>{item.titulo}{item.dataEnviado?"("+item.dataEnviado+")":<></>}</Link>}
